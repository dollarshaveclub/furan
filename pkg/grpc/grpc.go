...
func setTagsForSpan(span tracer.Span, build *furanrpc.BuildDefinition, push *furanrpc.PushDefinition, id gocql.UUID) {
...
func (gr *GrpcServer) buildDuration(ctx context.Context, req *furanrpc.BuildRequest) error {
...
func (gr *GrpcServer) pushDuration(ctx context.Context, req *furanrpc.BuildRequest, s3 bool) error {
...
func (gr *GrpcServer) totalDuration(ctx context.Context, req *furanrpc.BuildRequest) error {
	d, err := gr.durationFromStarted(ctx)
	if err != nil {
		return err
	}
...
func (gr *GrpcServer) syncBuild(ctx context.Context, req *furanrpc.BuildRequest) (outcome furanrpc.BuildStatusResponse_BuildState) {
	...
	defer func(id gocql.UUID) {
		failed = outcome == furanrpc.BuildStatusResponse_BUILD_FAILURE || outcome == furanrpc.BuildStatusResponse_PUSH_FAILURE
		flags := map[string]bool{
			"failed":   failed,
			"finished": true,
		}
		var eet furanrpc.BuildEventError_ErrorType
		if failed {
			eet = furanrpc.BuildEventError_FATAL
			gr.mc.BuildFailed(req.Build.GithubRepo, req.Build.Ref, errors.IsUserError(err))
			buildSpan.Finish(tracer.WithError(err))
		} else {
			eet = furanrpc.BuildEventError_NO_ERROR
		...
		event := &furanrpc.BuildEvent{
			EventError: &furanrpc.BuildEventError{
				ErrorType: eet,
				IsError:   eet != furanrpc.BuildEventError_NO_ERROR,
			},
			BuildId:       id.String(),
			BuildFinished: true,
			EventType:     furanrpc.BuildEvent_LOG,
			Message:       msg,
		}
	...
	if buildcontext.IsCancelled(ctx.Done()) {
		...
		return furanrpc.BuildStatusResponse_BUILD_FAILURE
	}
	err = gr.dl.SetBuildState(ctx, id, furanrpc.BuildStatusResponse_BUILDING)
	if err != nil {
	...
	if err != nil {
		if err == builder.ErrBuildNotNecessary {
			return furanrpc.BuildStatusResponse_NOT_NECESSARY
		}
	...
	err = gr.dl.SetBuildState(ctx, id, furanrpc.BuildStatusResponse_PUSHING)
	if err != nil {
	...
	if err != nil {
		...
		return furanrpc.BuildStatusResponse_PUSH_FAILURE
	}
	...
	err = gr.dl.SetBuildState(ctx, id, furanrpc.BuildStatusResponse_SUCCESS)
	if err != nil {
...
func (gr *GrpcServer) GetBuildStatus(ctx context.Context, req *furanrpc.BuildStatusRequest) (_ *furanrpc.BuildStatusResponse, err error) {
	parentSpan, _ := tracer.SpanFromContext(ctx)
	resp := &furanrpc.BuildStatusResponse{}
...
func (gr *GrpcServer) eventsFromDL(ctx context.Context, stream furanrpc.FuranExecutor_MonitorBuildServer, id gocql.UUID) (err error) {
...
func (gr *GrpcServer) eventsFromEventBus(stream furanrpc.FuranExecutor_MonitorBuildServer, id gocql.UUID) error {
	var err error
	output := make(chan *furanrpc.BuildEvent)
...
func (gr *GrpcServer) CancelBuild(ctx context.Context, req *furanrpc.BuildCancelRequest) (_ *furanrpc.BuildCancelResponse, err error) {
	...
	for i := 0; i < 5; i++ {
		...
		if gone {
			return &furanrpc.BuildCancelResponse{BuildId: req.BuildId}, nil
		}
	}
